<template>
    <div>
        <artifact-score-dialog v-if="showScore" v-model="showScore" :item="scoreItem" />
        <!-- new artifact dialog -->
        <add-artifact-dialog
                ...
                <el-dropdown trigger="click" @command="handleDropdownCommand">
                    ...
                    <template #dropdown>
                        <el-dropdown-menu>
                            ...
                            <el-dropdown-item command="exportShare" :icon="IconFa6SolidShareNodes">{{ isNative ? '保存分享文件' : t("artPage.shareLink") }}</el-dropdown-item>
                        </el-dropdown-menu>
                    </template>
                </el-dropdown>
            </el-button-group>
                    ...
                    <el-dropdown split-button size="small" @click="handleOutputJsonClicked" @command="handleOutputCommand">
                        ...
                        <template #dropdown>
                            <el-dropdown-menu>
                                ...
                                <el-dropdown-item command="share">{{ isNative ? '保存分享文件' : t("artPage.shareLink") }}</el-dropdown-item>
                            </el-dropdown-menu>
                        </template>
                    </el-dropdown>
                </el-button-group>
                ...
                <div v-if="filteredArtifacts.length > 0 && activeName === tab.name">
                    <div class="artifacts-div mona-scroll-hidden">
                        <artifact-display
                            ...
                            :edit-button="true"
                            selectable
                            @click="scoreArtifactId = item.id; showScore = true"
...
import { isNative, saveText } from '@/platform/native.mjs'
...
const scoreArtifactId = ref(-1)
const scoreItem = computed(() => artifactStore.artifacts.value.get(scoreArtifactId.value))
...
function shareArtifact() {
    if (isNative) { saveText(getArtifactString(), 'application/json', 'mona-artifacts.json').catch(e => ElMessage.error(e.message)); return }
    ElNotification({
