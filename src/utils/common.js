...
export async function downloadString(text, fileType, filename) {
    const { isNative, saveText } = await import('@/platform/native.mjs')
    if (isNative) return saveText(text, fileType, filename)
