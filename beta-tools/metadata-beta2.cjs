...
config:[bool('stance','巡风列装',true),bool('radiance','辉映·星扩散（需已触发冰扩散）'),int('disciplinary_stacks','整肃层数（二命列装自动满层）',0,6),int('anemo_cryo_count','队伍冰/风人数（包含薇斯纳）',1,4,1),int('other_count','队伍其他元素人数',0,3,1)],configSkill:[]};
...
targets.VesnaDefault={name:'VesnaDefault',nameLocale:t('薇斯纳-灵剑·爆发（测试服）'),description:t('最大化单次Q灵剑期望伤害，按角色的辉映状态切换普通风伤/直接星扩散；读取整肃、命座、队友BUFF。不是整轮DPS。旧定额位置开关仅供旧方案导入，现统一按已核对公式结算。'),tags:[],for:'Vesna',badge:image,config:[]};
