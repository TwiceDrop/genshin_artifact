...
impl WeaponCommonData {
    pub fn new(name: WeaponName, level: i32, ascend: bool, refine: i32) -> WeaponCommonData {
        ...
        let base_atk = crate::weapon::expanded_stats::stats(name, level, ascend)
            .or_else(|| crate::weapon::signature_stats::stats(name, level, ascend))
            .or_else(|| crate::weapon::snezhnaya_stats::stats(name, level, ascend))
...
impl<T: Attribute> ChangeAttribute<T> for WeaponCommonData {
    fn change_attribute(&self, attribute: &mut T) {
        ...
        if let Some(s) = self.static_data.weapon_sub_stat {
            ...
            if let Some((_, value)) = crate::weapon::expanded_stats::stats(self.name, self.level, self.ascend)
                .or_else(|| crate::weapon::signature_stats::stats(self.name, self.level, self.ascend))
                .or_else(|| crate::weapon::snezhnaya_stats::stats(self.name, self.level, self.ascend)) {
