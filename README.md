...
原神 7.1 桌面与安卓计算器。沃雅妮莎、薇斯纳及专武，以及「新枝」「凝雪沉心」「柔风游弦」和至冬纪行、锻造武器均已接入。详细配置见[使用说明](7.1.06使用说明.md)。
...
- 本版是部分修复：新增套装、星/月反应、理论排行及旧角色使用新武器的部分路径仍受限制，详见[更新说明](7.1.06更新说明.md)。
- 保留米游社导入、UID 数据管理、伤害对比与词条收益等功能。

[下载 7.1.06 电脑版、安卓版](https://github.com/TwiceDrop/genshin_artifact/releases/tag/v7.1.06) · [更新说明](7.1.06更新说明.md) · [武器数据与范围](beta-data/limited-71-notes.md) · [原项目](https://github.com/wormtql/genshin_artifact)

安卓版沿用原版包名与签名，可覆盖安装并保留本机数据；安装与互传说明见[安卓使用说明](docs/android-offline.md)。

## 启动

Windows 安装包自带 Node.js，无需另装；也可下载网页压缩包，安装 Node.js 22 LTS 或更新版本后解压双击 `启动7.1.06.bat`，打开 http://127.0.0.1:4183/#/calculate 。从源码启动：
...
7.1.06 修复奥黛塔、BUFF配置兼容、后台条件与队友装备占用；具体适配范围和仍保留的限制见更新说明。
